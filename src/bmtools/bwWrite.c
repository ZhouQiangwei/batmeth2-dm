...
int bwAddIntervals(bigWigFile_t *fp, char **chrom, uint32_t *start, uint32_t *end, float *values, uint16_t *coverage, uint8_t *strand,
    ...
    /*
    if(fp->type & BM_COVER){
        if(!memcpy(wb->p+wb->l+8, coverage, sizeof(uint16_t))) return 9;
        if(!memcpy(wb->p+wb->l+10, strand, sizeof(uint8_t))) return 9;
        if(!memcpy(wb->p+wb->l+11, context, sizeof(uint8_t))) return 9;
        updateStats(fp, end[0]-start[0], values[0]);
        wb->l += 12;
    }else if (fp->type == 1){
        if(!memcpy(wb->p+wb->l+4, end, sizeof(uint32_t))) return 8;
        if(!memcpy(wb->p+wb->l+8, values, sizeof(float))) return 9;
        if(!memcpy(wb->p+wb->l+12, coverage, sizeof(uint16_t))) return 9;
        if(!memcpy(wb->p+wb->l+14, strand, sizeof(uint8_t))) return 9;
        if(!memcpy(wb->p+wb->l+15, context, sizeof(uint8_t))) return 9;
        updateStats(fp, end[0]-start[0], values[0]);
        wb->l += 16;
    }else if(fp->type == 2){
        if(!memcpy(wb->p+wb->l+4, end, sizeof(uint32_t))) return 8;
        if(!memcpy(wb->p+wb->l+8, values, sizeof(float))) return 9;
        if(!memcpy(wb->p+wb->l+12, coverage, sizeof(uint16_t))) return 9;
        fprintf(stderr, "2entryid %s\n", entryid[0]);
        slen = strlen(entryid[0]) + 1;
        if(!memcpy(wb->p+wb->l+14, entryid[0], sizeof(char*))) return 9;
        updateStats(fp, end[0]-start[0], values[0]);
        wb->l += 14;
        wb->l += slen;
    }else if(fp->type == 3){
        ...
        if(!memcpy(wb->p+wb->l+8, values, sizeof(float))) return 9;
        if(!memcpy(wb->p+wb->l+12, coverage, sizeof(uint16_t))) return 9;
        fprintf(stderr, "4entryid %s\n", entryid[0]);
        if(!memcpy(wb->p+wb->l+14, strand, sizeof(uint8_t))) return 9;
        if(!memcpy(wb->p+wb->l+15, context, sizeof(uint8_t))) return 9;
        slen = strlen(entryid[0]) + 1;
        if(!memcpy(wb->p+wb->l+16, entryid[0], sizeof(char*))) return 9;
        updateStats(fp, end[0]-start[0], values[0]);
        wb->l += 16;
        wb->l += slen;
    }
    */
    ...
    for(i=1; i<n; i++) {
        ...
        if(wb->l+50 > fp->hdr->bufSize) { //12 bytes/entry, no now 19 bytes/entry
            wb->end = end[i-1];
            flushBuffer(fp);
            wb->start = start[i];
        }
...
/*
        if(fp->type == 0){
            if(!memcpy(wb->p+wb->l+4, &(values[i]), sizeof(float))) return 13;
            if(!memcpy(wb->p+wb->l+8, &(coverage[i]), sizeof(uint16_t))) return 13;
            if(!memcpy(wb->p+wb->l+10, &(strand[i]), sizeof(uint8_t))) return 13;
            if(!memcpy(wb->p+wb->l+11, &(context[i]), sizeof(uint8_t))) return 13;
            updateStats(fp, end[i]-start[i], values[i]);
            wb->l += 12;
        }else if(fp->type == 1){
            if(!memcpy(wb->p+wb->l+4, &(end[i]), sizeof(uint32_t))) return 12;
            if(!memcpy(wb->p+wb->l+8, &(values[i]), sizeof(float))) return 13;
            if(!memcpy(wb->p+wb->l+12, &(coverage[i]), sizeof(uint16_t))) return 13;
            if(!memcpy(wb->p+wb->l+14, &(strand[i]), sizeof(uint8_t))) return 13;
            if(!memcpy(wb->p+wb->l+15, &(context[i]), sizeof(uint8_t))) return 13;
            updateStats(fp, end[i]-start[i], values[i]);
            wb->l += 16;
        }else if(fp->type == 2){
            if(!memcpy(wb->p+wb->l+4, &(end[i]), sizeof(uint32_t))) return 12;
            ...
            fprintf(stderr, "2entryid %s\n", entryid[i]);
            slen = strlen(entryid[i]) + 1;
            if(!memcpy(wb->p+wb->l+14, entryid[i], sizeof(char*))) return 13;
            updateStats(fp, end[i]-start[i], values[i]);
            wb->l += 14;
            wb->l += slen;
        }else if(fp->type == 3){
            fprintf(stderr, "3entryid %s\n", entryid[i]);
            if(!memcpy(wb->p+wb->l+4, &(end[i]), sizeof(uint32_t))) return 12;
            if(!memcpy(wb->p+wb->l+8, &(values[i]), sizeof(float))) return 13;
            if(!memcpy(wb->p+wb->l+12, &(coverage[i]), sizeof(uint16_t))) return 13;
            updateStats(fp, end[i]-start[i], values[i]);
            ...
            if(!memcpy(wb->p+wb->l+15, &(context[i]), sizeof(uint8_t))) return 13;
            slen = strlen(entryid[i]) + 1;
            if(!memcpy(wb->p+wb->l+16, entryid[i], sizeof(char*))) return 13;
            updateStats(fp, end[i]-start[i], values[i]);
            wb->l += 16;
            wb->l += slen;
        }
...
int bwFinalize(bigWigFile_t *fp) {
    ...
    if(DEBUG>1) printf("4444\n");
    //write the summary information
    if(writeSummary(fp)) return 3;

    if(DEBUG>1) printf("444477777\n");
