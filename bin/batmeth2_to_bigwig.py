...
def processInputs( allCFileAr, chrmFileStr, keepTmp, labelsAr, outID, baseMod, numProc, isSort ,printStrand, fileformat):
	...
	results = [ pool.apply_async( processFile, args=(allCFileAr[i], chrmFileStr, labelsAr[i], outID, baseMod,  keepTmp, isSort,printStrand, fileformat) ) for i in range(len(allCFileAr)) ]
	suc = [ p.get() for p in results ]

	print( 'Done' )


def processFile( allCFileStr, chrmFileStr, label, outID, baseMod, keepTmp, isSort, printStrand, fileformat):
	...
	readAllC( allCFileStr, bedGraphAr, OutCfileStr ,printStrand, fileformat)
...
def readAllC( allCFileStr, outFileAr, OutCfileStr, printStrand, fileformat):
	...
	for line in allCFile:
		...
		if line.startswith('#') or len(lineAr) < 7:  #or lineAr[6].isdigit() == False:
			...
			if isContext:
				methType = decodeMethType( context )
				try:
					mInd = mTypes.index( methType )
				except ValueError:
					continue
			value = float( coverC ) / float( coverCT )
			if value > 1:
				value = 1.0
			#if value == 0:  #skip 0
			#	continue
...
def parseInputs( argv ):
	...
	for i in range( min(5, len(argv)-2) ):
		if argv[i] == '-keep':
			...
			printStrand = True
			startInd += 1
		elif argv[i].startswith( '--bedmethyl' ):
			fileformat = "bedmethyl"
			startInd += 1
	...
	processInputs( allCFileAr, chrmFileStr, keepTmp, labelsAr, outID, baseMod, numProc, isSort ,printStrand, fileformat)
...
def printHelp():
	...
	print( '-bedmethyl\tbedmethyl file format [default methratio]' )
